...
```php PHP
<?php
$api_key = 'buderim_your_api_key_here';
...
async function updateRateDisplay() {
  try {
    const response = await fetch('https://api.exchangeratesapi.com.au/latest', {
      headers: { 'Authorization': 'Bearer buderim_your_api_key_here' }
    });
...
results = calculate_portfolio_values(portfolio, 'buderim_your_api_key_here')
...
// Example usage
const monitor = new RateMonitor('buderim_your_api_key_here');
...
// Usage
$cache = new ExchangeRateCache('buderim_your_api_key_here');
