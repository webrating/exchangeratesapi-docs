...
The Exchange Rates API is a developer-friendly data service providing **official Reserve Bank of Australia (RBA) exchange rate data** through clean, modern REST API endpoints and webhooks. We transform publicly available RBA data (currently only accessible via HTML/RSS) into instant JSON responses.
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="gift"
  >
    300 requests to get started (one-time trial), no credit card required
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  ...
  <Card
    title="Webhooks (Professional+)"
    icon="bell"
    href="/api-reference/webhooks/overview"
  >
    Get pushed a signed notification the moment each day's rates publish - no polling
  </Card>
</CardGroup>