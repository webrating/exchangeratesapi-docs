...
- **Webhooks (preferred for freshness)** - on the Professional plan and above, register a [webhook](/api-reference/webhooks/overview) and we push you a signed notification within minutes of publication. You fetch exactly once, when the data is actually ready, instead of polling and guessing.
...
class QuotaManager {
  ...
  async makeRequest(endpoint) {
    ...
    if (this.quotaInfo.remaining !== null && this.quotaInfo.remaining <= 0) {
      ...
      if (new Date() < resetTime) {
        throw new Error(`Monthly quota exceeded. Resets at ${resetTime.toLocaleString()}`);
      }
    }
  ...
  updateQuotaInfo(headers) {
    // Quotas are monthly. The API returns X-RateLimit-*-Monthly plus a reset timestamp.
    const limit = headers.get('X-RateLimit-Limit-Monthly');
    const remaining = headers.get('X-RateLimit-Remaining-Monthly');
